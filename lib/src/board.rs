    ...
    pub fn make_move(&mut self, m: Move) -> bool {
        ...
        if m.capture {
            ...
            if self.side_to_move == Color::White {
                if m.to == Square::H8 {
                    self.can_castle &= !(Castle::BlackKingSide as u8)
                } else if m.to == Square::A8 {
                    self.can_castle &= !(Castle::BlackQueenSide as u8)
                }
            } else {
                if m.to == Square::H1 {
                    self.can_castle &= !(Castle::WhiteKingSide as u8)
                } else if m.to == Square::A1 {
                    self.can_castle &= !(Castle::WhiteQueenSide as u8)
                }
            }
        }
        match m.special {
        ...
        if m.piece.kind == PieceKind::King {
            match self.side_to_move {
                Color::White => {
                    self.can_castle &= !(Castle::WhiteQueenSide as u8 | Castle::WhiteKingSide as u8)
                }
                Color::Black => {
                    self.can_castle &= !(Castle::BlackQueenSide as u8 | Castle::BlackKingSide as u8)
                }
            }
        }
...
impl Board {
    ...
    pub fn from_fen(fen: &str) -> Board {
        ...
        for c in castling.chars() {
            match c {
                'K' => s.can_castle |= Castle::WhiteKingSide as u8,
                'Q' => s.can_castle |= Castle::WhiteQueenSide as u8,
                'k' => s.can_castle |= Castle::BlackKingSide as u8,
                'q' => s.can_castle |= Castle::BlackQueenSide as u8,
                '-' => (),
                _ => panic!("Invalid FEN"),
            }
        }
...
impl std::fmt::Display for Board {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "  En passant: {:?}",
            self.en_passant.map(|x| Square::from(x))
        )?;
        write!(f, "  Can castle: ")?;
        [
            (Castle::WhiteKingSide, "K"),
